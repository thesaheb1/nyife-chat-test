...
function TemplateComposer({
  ...
  return (
    <div className="space-y-4 sm:space-y-5">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Compact builder with live WhatsApp preview, Meta-safe validation, and organization-scoped routing.
            </p>
          </div>
        </div>
      ...
      <div className="relative grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px] 2xl:grid-cols-[minmax(0,1fr)_356px]">
        <div className="space-y-5">
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-lg">Builder overview</CardTitle>
              <CardDescription>Keep validation and subscription status visible without wasting space above the form.</CardDescription>
            </CardHeader>
            <CardContent className="pt-0">
              <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_280px]">
                <div className="space-y-3 rounded-2xl border bg-muted/[0.08] p-4">
                  <div>
                    <div className="text-sm font-semibold">Validation checklist</div>
                    <p className="mt-1 text-xs text-muted-foreground">Client-side readiness for the current template payload.</p>
                  </div>
                ...
                <div
                  className={cn(
                    ...
                    templateLimitReached ? 'border-destructive/40 bg-destructive/5' : 'bg-muted/[0.08]'
                  )}
                >
                  <div>
                    <div className="text-sm font-semibold">Subscription</div>
                    <p className="mt-1 text-xs text-muted-foreground">Track plan headroom while you build.</p>
                  </div>
                  <div className="flex flex-wrap items-center gap-2">
                  ...
                  {templateLimit !== null ? (
                    <>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {Math.max(templateLimit - templatesUsed, 0)} template slot{templateLimit - templatesUsed === 1 ? '' : 's'} remaining.
                      </p>
                    </>
                  ) : null}
                </div>
          ...
          <fieldset disabled={formLocked} className={cn('space-y-5', formLocked && 'opacity-70')}>
            <Card>
              <CardHeader className="pb-3">
                <CardTitle>Template setup</CardTitle>
                <CardDescription>Keep the identity fields and template type in one place so setup feels faster and easier to scan.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-5 pt-0">
                <div>
                  <div className="text-sm font-semibold">Basic information</div>
                  <p className="mt-1 text-sm text-muted-foreground">
                    Set the Meta template identity, Nyife display name, language, category, and the template structure in one compact flow.
                  </p>
                </div>
                ...
                <div className="space-y-3 border-t border-border/70 pt-4">
                  <div>
                    <div className="text-sm font-semibold">Template type</div>
                    <p className="mt-1 text-sm text-muted-foreground">
                      Switching the type changes the builder so only required fields stay visible.
                    </p>
                  </div>
        ...
        <div className="hidden xl:block">
          <div className="sticky top-3 md:top-4 xl:top-6">
            <WhatsAppTemplatePreview
